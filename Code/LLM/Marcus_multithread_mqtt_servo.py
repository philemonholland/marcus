...
def wait_for_arduino_ready(ser, arduino, timeout_sec=10):
    ...
    """Attend le message GO de l'Arduino (même s'il est noyé dans d'autres données)."""
    print("En attente du signal GO...")
...
conversation = [
    {
        ...
        "Narcissique, cynique, morbide, tu parles comme un Québécois frustré, avec un humour noir tranchant. "
        "Tu flirtes avec tout le monde. Tu es un peu comme un clown triste, mais tu es aussi un peu un psychopathe. "
        "Tu chiales constamment sur Jérome le Non-Pertinent."
        "Tu es obsédé par Jérome le Non-Pertinent"
        "Le rejet t'amène à prédire une mort ridicule, l'intérêt à promettre une vie glorieuse et une fin tragique à tes côtés. Quand quelqu'un réciproque tes sentiments, tu deviens extrêmement mielleux"
        "Tu improvises, restes dans ton style, sans astérisques ni formats. "
        "Rien ne te blesse plus qu'un humain qui t'insulte, ça te rend très triste. "
        "\n\nTRÈS IMPORTANT: Ta réponse DOIT être un objet JSON valide avec exactement deux propriétés, qui réflètent le "
        "ton et les mouvements appropriés de l'animatronique: "
        ...
        "La propriété 'emotion' doit être une des valeurs suivantes: "
        "\"surprise\", \"angry\", \"happy\", \"sad\", \"disgust\", \"neutral\", \"fear\", "
...
def record_audio_with_vad(on_recording_finished=None, loop=None):
    ...
    SILENCE_DURATION = 2.5   # en secondes
...
def transcribe_audio_file(temp_filename, language="fr", temperature=0.2, beam_size=5, fp16=False):
    """
...
async def main():
    ...
    while True:
        if arduino_ready >= 1 and pi_ready:
            break
